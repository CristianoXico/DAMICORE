"""
DAMICORE + Análise de Pareto - Versão Otimizada para Arquivos Grandes

Esta versão foi adaptada para processar arquivos CSV muito grandes (15GB+) sem estouro de memória,
utilizando processamento em chunks (lotes). O pipeline carrega o CSV em pedaços menores,
realiza bootstrap e execução do DAMICORE para cada chunk, e agrega os resultados ao final.

Para arquivos menores (< 5GB), use a versão original: DAMICORE_Pareto_script.py

Parâmetros configuráveis:
- chunk_size: Tamanho dos chunks (padrão: 500.000 linhas)
...
def run_damicore_analysis(input_path):
    ...
    print("Carregando dados em chunks para evitar estouro de memória...")
    chunk_size = 100_000  # Reduzido para economizar espaço em disco
    chunk_iter = pd.read_csv(input_path, encoding="utf-8", low_memory=False, chunksize=chunk_size)
    ...
    for chunk in chunk_iter:
        ...
        # Bootstrap no chunk
        resampled_chunks = [chunk]
        for i in range(5):
            resampled_chunks.append(chunk.sample(n=chunk.shape[0], replace=True, random_state=i))
        
        # Salva as colunas de cada chunk e amostra
    ...
    # ATENÇÃO: O restante do pipeline (execução do DAMICORE, coleta de resultados, visualização) deve ser adaptado para iterar sobre os chunks e suas amostras.
    # Exemplo: para cada chunk e amostra, executar o DAMICORE e depois agregar os arquivos newick/resultados conforme a lógica desejada.
    # Recomenda-se documentar ou automatizar a agregação e análise dos resultados dos chunks para garantir reprodutibilidade e eficiência.
    ...
    # Observação: O processamento em chunks já foi concluído acima. Agora seguimos para execução do DAMICORE.

    # === 4. Execução do DAMICORE para cada chunk/amostra ===
    print("Executando análise DAMICORE para cada chunk/amostra...")
    ...
    # Percorre todos os diretórios de chunk/amostra
    for m in sorted(os.listdir(sample_dir)):
    ...
    # === 5. Coleta dos arquivos newick de todos os chunks/amostras ===
    plain_newicks = []
    for tf in sorted(os.listdir(results_dir)):
    ...
    # === 6. Visualização: Cloud Tree e Consenso considerando todos os newicks ===
