#!/usr/bin/env python3
"""
damicore_consensus_clade_analyzer_v5.5.py

🔥 Versão 5.5 - Correção final do mapeamento (remoção de aspas simples). Clados de tamanho variável (>=2), mínimo de 20 newicks, sem filtro global.
...
def translate_label(label, mapping):
    ...
    if label is None:
        return None
    # Remove as aspas simples que o Biopython pode adicionar ao ler o Newick
    s = str(label).replace("'", "")
    if not mapping:
        return s

    # 1. Tenta a chave exata (string)
    if s in mapping:
        return mapping[s]

    # 2. Tenta a chave numérica (se o rótulo for apenas o número)
    try:
        ki = int(s)
        if ki in mapping:
            return mapping[ki]
    ...
    # 3. Tenta extrair o número do rótulo (ex: 'col_0.txt' -> 0)
    digits = re.sub(r"\D", "", s)
    if digits:
        try:
            di = int(digits)
    ...
    # 4. Tenta o rótulo sem aspas/espaços (apenas para robustez, mas o passo 1 deve pegar)
    s_clean = s.strip().replace('"', "")
    if s_clean in mapping:
        return mapping[s_clean]

    # Se falhar, retorna o rótulo original
    return s
...
def write_markdown(summary_df, out_path):
    lines = []
    lines.append("# Sumário Global de Clados (v5.5)\n")
...
def main():
    print(
        "DAMICORE Consensus Clade Analyzer v5.5 — Clados >=2, 20+ newicks, sem filtro global, mapeamento corrigido"
    )
