...
def analyze_newicks(
    ...
    var_mapping_csv: opcional, CSV com colunas [original, mapped, category] para traduzir nomes de variáveis
    ...
    # opcional: var_mapping para traduzir nomes de variáveis
    var_map = {}  # original -> mapped
    category_map = {}  # original -> category
    if var_mapping_csv and os.path.exists(var_mapping_csv):
        try:
            vm = pd.read_csv(var_mapping_csv, dtype=str)
            # espera colunas: original, mapped, category
            for _, r in vm.iterrows():
                orig = r.get("original", "").strip()
                mapped = r.get("mapped", "").strip()
                cat = r.get("category", "—").strip()
                if orig:
                    var_map[orig] = mapped
                    category_map[orig] = cat
        except Exception:
            pass  # se falhar, continua sem mapeamento

    # estruturas para contagem
    clade_stats = defaultdict(
        lambda: {
    ...
    for cl, stats in clade_stats.items():
        ...
        # Traduzir variáveis do clado se tivermos var_map
        def translate_term(t: str) -> str:
            ...
            # try exact match
            if t_str in var_map:
                return var_map[t_str]
            # try basename without path
            b = os.path.basename(t_str)
            if b in var_map:
                return var_map[b]
            # try without extension
            noext = os.path.splitext(b)[0]
            if noext in var_map:
                return var_map[noext]
            # fallback: return original
            return t_str

        translated = [translate_term(x) for x in cl]

        # Categoria: tentar mapear por primeira variável presente
        categoria = "—"
        for key in cl:
            if key in category_map:
                categoria = category_map[key]
                break
            b = os.path.basename(key)
            if b in category_map:
                categoria = category_map[b]
                break

        # Level representativo: 3 se freq_3 >= freq_2 else 2
        level_rep = 3 if freq_3 >= freq_2 else 2

        # stable id (determinístico): md5 of joined terminals
        m = hashlib.md5()
        joined = ",".join(cl)
        m.update(joined.encode("utf-8"))
        clade_id = m.hexdigest()

        rows.append(
            {
                "Clade_ID": clade_id,
                "Clado": ",".join(cl),
                "Clade_Variables": ",".join(translated),
                "Frequency": presence,
                "Level": level_rep,
                "Total_Resamples": n_files,
                "Relative_Frequency (%)": round(presence_frac * 100, 2),
    ...
    df = df.sort_values(
        by=["Ocorrências (Estratificações)"],
        ascending=False,
        key=lambda s: s.map(
    ...
    headers = [
        "Clado",
        "Ocorrências (Estratificações)",
        "Regiões analisadas",
        "Observações de Frequência",
        "Categoria",
        "Clados Representativos",
        "Estabilidade Global",
    ]
    ...
    print(
        f"  Clados na Lista Principal: {len(lista_principal)} (salve {csv_out} para ver detalhes)"
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Uso: python analyze_newicks.py /caminho/para/dir_newicks [metadata.csv] [var_mapping.csv]")
        print()
        print("Argumentos:")
        print("  /caminho/para/dir_newicks : Diretório contendo arquivos .newick")
        print("  [metadata.csv]            : (Opcional) CSV com mapeamento filename->region")
        print("  [var_mapping.csv]         : (Opcional) CSV com mapeamento original->mapped->category")
        print("                               Se não fornecido, tenta usar var_mapping.csv do diretório atual")
        sys.exit(1)
    ...
    # Se temos 2 argumentos, pode ser metadata ou var_mapping
    # Se temos 3 argumentos, é metadata e var_mapping
    if len(sys.argv) > 2:
        arg2 = sys.argv[2]
        # Se arg2 termina com .csv e contém "mapping" ou "var_", considerar como var_mapping
        # Caso contrário, considerar como metadata
        if "var_mapping" in arg2.lower() or "mapping" in arg2.lower():
    ...
    elif not var_mapping_file:
        # Tentar usar var_mapping.csv do diretório atual como padrão
        default_mapping = "var_mapping.csv"
        if os.path.exists(default_mapping):
            var_mapping_file = default_mapping
    
    # Chamar a função com os três parâmetros
