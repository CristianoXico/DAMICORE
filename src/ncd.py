...
def interleave(fname1, fname2, block_size = 1024,
  ...
  interleave_name = os.path.basename(fname1) + '--' + os.path.basename(fname2)
  interleave_fname = os.path.join(pair_dir, interleave_name)

  maxsize = max(os.path.getsize(fname1), os.path.getsize(fname2))
  with open(interleave_fname, 'wb') as f,\
      open(fname1, 'rb') as i1,\
      open(fname2, 'rb') as i2:
